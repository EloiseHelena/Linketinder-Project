...
function curtirVaga(vagaTitulo: string) {
    let curtidasCandidato = JSON.parse(localStorage.getItem('curtidasCandidato') || '[]');
...
function curtirCandidato(candidatoNome: string) {
    let curtidasEmpresa = JSON.parse(localStorage.getItem('curtidasEmpresa') || '[]');
...
function verificarMatch(item: string) {
  let curtidasCandidato = JSON.parse(localStorage.getItem('curtidasCandidato') || '[]');
  let curtidasEmpresa = JSON.parse(localStorage.getItem('curtidasEmpresa') || '[]');
...
function notificarCurtida(item: string) {
    Swal.fire({
      title: 'Sucesso!',
  ...
  function notificarMatch(item: string) {
    Swal.fire({
...
function atualizarVagaRevelada(vaga: Vaga) {
    const vagaDiv = document.getElementById(`vaga-${vaga.titulo}`);
    if (vagaDiv) {
      const empresaSpan = vagaDiv.querySelector('.empresa') as HTMLElement; 
      if (empresaSpan) {
  ...
  function atualizarCandidatoRevelado(candidato: Candidato) {
    const candidatoDiv = document.getElementById(`candidato-${candidato.nome}`);
    if (candidatoDiv) {
      const nomeSpan = candidatoDiv.querySelector('.nome') as HTMLElement; 
      if (nomeSpan) {
...
function exibirCurtidasCandidato() {
  ...
  if (listaCurtidasCandidato) {
    ...
    if (curtidasCandidato.length === 0) {
      ...
      curtidasCandidato.forEach((vaga: string) => {
...
function exibirCurtidasEmpresa() {
  ...
  if (listaCurtidasEmpresa) {
    ...
    if (curtidasEmpresa.length === 0) {
      ...
      curtidasEmpresa.forEach((candidato: string) => {
        const li = document.createElement('li');
...
function carregarDadosRevelados() {
  ...
  vagasReveladas.forEach((vaga: Vaga) => {
    atualizarVagaRevelada(vaga);
  });

  candidatosRevelados.forEach((candidato: Candidato) => {
...
function recomendarVagas(candidato: Candidato, vagas: Vaga[]): Vaga[] {
    return vagas
      .map(vaga => ({
        vaga,
...
function exibirVagasRecomendadas(candidato: Candidato, vagas: Vaga[]) {
